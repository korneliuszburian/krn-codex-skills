...
waits for nothing. You fix the question, choose the surface, prepare its
evidence, assemble the prompt from the reference, hand it to the
...
1. **Fix the question and the scope.** Name the decision the analysis must
   inform, its source and fixed point, and what it must not decide. For a
   repository code question, include the branch, commit, paths or symbols.
   If the question is vague, write the
   sharpest version you can and let the prompt ask Astra for the rest in its
   final section.
   **Done when:** one question, its source and fixed point, and the
   forbidden decisions are explicit.

2. **Choose the surface and the model settings.** Read
...
3. **Prepare evidence for that surface.** A GitHub connector reads a remote
   commit, never the working tree. Name an already published SHA when it answers
   the question. If the question requires unpublished changes, obtain separate
   commit and push authority before relying on the connector, or select an
   authorized attachment and state its limits. A public-source or snippet
   question needs no repository publication. Never publish secrets.
   **Done when:** the selected surface can see the named evidence and any
   invisible local state is an explicit non-proof.
...
4. **Assemble the prompt from the contract.** Build the six blocks in order from
   ...
   generator. Apply [`references/project-setup.md`](references/project-setup.md)
   only when shared project context is needed across requests.
   **Done when:** the prompt carries all six blocks, pins the applicable fixed point, and fixes
   the answer schema.
...
5. **Hand over and retrieve.** The operator pastes the prompt into the chat and
   ...
   hang.
   **Done when:** the operator has the prompt, or a written blocker names the
   unavailable selected surface or evidence.
...
- [`references/project-setup.md`](references/project-setup.md) — optional shared
  ChatGPT project context and connector scope for repeated questions.

---
name: slice-work
description: Turn a settled spec into implementation-ready vertical slices for implement, each one-fresh-session-sized with explicit blocking dependencies and its own decision evidence. Use before implementation to decompose a settled outcome; skip unresolved fog, single edits, and execution.
---

# Slice Work

Produce the slice list; never execute it. `implement` owns one slice,
`delivery-loop` owns the lifecycle, `domain-modeling` owns vocabulary. This skill
only **decomposes** a settled outcome into one-fresh-session vertical slices.
...
7. **Emit the list and hand off.** Output the slice list with the dependency
   graph and a "demonstrable after slice K" marker. Hand each slice to
   `$implement` in a fresh context; `$delivery-loop` owns lifecycle and claim
   state. This skill stores nothing durable.
   ...
   **Done when:** every slice is one-fresh-session-sized, end-to-end vertical,
   has explicit dependencies, carries its own decision evidence, and none is a
   horizontal layer.

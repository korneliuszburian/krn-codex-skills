...
- Every update names the owner, evidence, and next action; never claim completion without observed evidence or readback; a deterministic falsifier outranks a model review, same-model self-assessment is never proof, and a new falsifier must be shown failing first.
- A harness-surface commit names a `Change-contract: <check>:red->green`. A changed or new observer needs the observed before-state (`--before`); a behavior-preserving change keeps an unchanged check green. `npm run changes:check` blocks an unmet prediction and names the commit to revert.

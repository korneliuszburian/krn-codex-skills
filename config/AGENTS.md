...
- Every update names the owner, the evidence, and the next action; never claim completion without observed evidence or readback; a deterministic falsifier outranks a model review, same-model self-assessment is never proof, and each new falsifier must be shown failing before it counts.
- A harness-surface commit (runtime lib, gate, workflow, or package manifest) names a `Change-contract: <check>:red->green` prediction. A changed or new observer requires the observed before-state (`--before`); a behavior-preserving change instead keeps an unchanged check green (declare `red->green` only for a claimed behavioral flip). `npm run changes:check` blocks an unmet prediction and names the commit to revert.

...
- Repository memory: the transient outcome capsule lives at `.krn/runs/delivery-loop/<outcome-id>/state.md` with `$delivery-loop` as sole writer, checked by `krn-codex state check`/`state resume`; durable shared truth lives in `CONTEXT.md`, `docs/adr/`, and `docs/research/`; cross-run workflow lessons live in `docs/research/workflow-lessons.md` (record a recurrence by appending `YYYY-MM-DD@<7-hex>` to the existing row and consolidating the class into one row; a recurring row also names the `Falsifier` `<test>/<file>.mjs::<case>@<7-hex>` that proved its gate, re-run by `npm run lessons:verify`; a recurrence after that proof fails closed where the commit exists, since a proof commit absent from this checkout is provenance, not ancestry-checked; a row may carry a `path:`, `symbol:`, or `churn:` `Trigger`, delivered by `krn-codex memory recall` (`--changed` for path/churn, `--symbol` for symbols) and required to be reconstructed with a `Recall: <gate> => <changed file or symbol>` trailer in a matching change, with the recalled lesson's test declared among the change's checks when it has one; a row may be retired with a `retired@<7-hex>` `Status` that names a supersession or a removed gate, and only active rows count against the cap); native Goals, transcripts, and chat history are not repository knowledge, and personal ChatGPT/Codex memory is outside repository knowledge — answer "how does memory work here" from this contract and those pages, not from external docs or the `openai-docs` skill.

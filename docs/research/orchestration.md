...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; the first direct catalog tests cover toggle, no-op, and quarantine with a mutation red then green; luna review raised two trailing-blank findings, repaired at ebd8afd and re-reviewed with no finding | continue with catalog-usage and catalog-inventory, then the validate rule remainder | catalog-usage (944) and catalog-inventory (903) remain monolithic and untested, catalog-document (580) can still split parser from operations, and review sandboxes block git-spawning tests |

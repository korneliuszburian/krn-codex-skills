...
| MAST-derived review checklist for specification and verification failures | lab-test | `$delivery-loop` on the next non-mechanical review; a seeded-defect review must catch a specification or incomplete-verification defect with the checklist and miss it without, or the checklist is deleted | no always-loaded checklist and no multi-agent redesign; the taxonomy is descriptive, not KRN evidence |

...
| Local architecture review | a read-only sub-agent turned 14 findings into 7 deepening candidates (capsule ABI, lessons schema, install trust, validator rules, diagnostics, export index, catalog identity); candidate 1 shipped at 88ab680 as `scripts/lib/capsule-abi.mjs` with `test/capsule-abi.test.mjs` and a mutation proof (30/31 red, 31/31 restored), and an independent luna review returned no finding with file:line citations and `npm run validate` PASS | keep the report-driven, candidate-by-candidate pathway in the architecture-review capsule; candidates 2-7 remain to be dispositioned | one reviewer model and one sandbox with a git-spawning execution gap; the review could not run state tests |

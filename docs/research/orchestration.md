...
| Local architecture review | a read-only sub-agent turned 14 findings into 7 deepening candidates; candidates 1-6 shipped (capsule ABI 88ab680, lessons schema a86fc28, install trust dd98bad, durable-pages rule 4c40374, diagnostics and shared exit codes cc1b7d3 with repair 12d481e, skill metadata and marker set a9a621e); each had a mutation falsifier shown red then green; independent luna reviews returned two findings, both dispositioned (one repaired, one rejected with evidence) | keep the report-driven, candidate-by-candidate pathway in the architecture-review capsule; candidate 7 remains; the command registry is deferred until the CLI next changes | one reviewer model and a sandbox that blocks git-spawning tests |

...
| Multi-framing state-audit swarm | adopt as a signal, never as proof (measured 2026-09-11: a three-judge read-only swarm with different framings — adversarial gate attack, architecture/slimming, fresh-source scan — surfaced four real gate bypasses that the green suite did not, and one doc-drift finding) | maintainer and `$delivery-loop`; when measured topics run out, run a fresh-source scan then a swarm with different framings, disposition every finding, and repair or record a residual; agreement is evidence and every claim is confirmed by a deterministic falsifier before it counts | no judge fleet and no scoring pipeline; judge votes never substitute for a falsifier or an independent lab-test decider; a lane that cannot execute commands states that gap |

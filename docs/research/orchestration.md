...
| Executor policy: Codex-only lanes | adopt (user decision 2026-09-16: Codex is KRN's only executor; no other transport or model family is authorized for KRN lanes, labs, or worker sessions) | installed sessions and lab runners; a lane or reviewer that would need another transport stops and reports instead of substituting one | no cross-transport or cross-family lanes, no second-executor support, and no model-pool machinery |
| Local-only AFK lane mechanics | adopt (earned by the LT-7 phases 2026-09-16: a red-base assertion before any model call, a loadable red deciding check, literal per-lesson Recall/At-risk trailers, hook-seeded capsule continuation with forwarded run state, one integrator gating the merged fixed point, and capsule writeback; N=2..8 concurrent sessions with zero unexplained repairs) | future local lanes and the maintainer; a lane that cannot pass its worker and integrator gates with the documented pattern, or a repair the pattern does not cover, reopens the row | local commits only: no remote-write, no tracker control plane, no cross-transport executor, and no fleet daemon |

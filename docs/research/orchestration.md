...
| Memory hygiene: fail-closed staleness, provenance, and reachability checks on existing artifacts | adopt (earned by measured silent failures on 2026-09-11: an `ACTIVE` capsule ~25 commits behind HEAD returned `clean`, lesson-gate resolution claimed in `validate` was not wired there, an ADR had no inbound link, and workflow-lessons was absent from `CONTEXT.md`) | `krn-codex state check` / `$delivery-loop` / maintainer; a stale or contradictory capsule, a completed outcome still `ACTIVE`, an orphaned non-delivery run, a lesson whose only gate is `manual:`/host-specific, and a durable capability absent from the knowledge map must be blocking or explicitly disclosed; reopen when a bounded structural check cannot decide the case | no new store, no embeddings/graph, no generic poisoning filter, no LLM summarizer; content-only write screens are known to fail, so hygiene stays structural (fixed point, provenance, gates, links) |

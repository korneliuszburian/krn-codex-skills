...
| Local architecture review | a read-only sub-agent turned 14 findings into 7 deepening candidates; candidates 1-5 shipped (capsule ABI at 88ab680, lessons schema at a86fc28, install trust at dd98bad, durable-pages rule at 4c40374, diagnostics and shared exit codes at cc1b7d3 with a repair at 12d481e); every candidate had a mutation falsifier shown red then green, and independent luna reviews returned two findings: one accepted and repaired, one rejected with the repository's convention or a sandbox execution gap | keep the report-driven, candidate-by-candidate pathway in the architecture-review capsule; candidates 6-7 remain; the command registry is deferred until the CLI next changes | one reviewer model and a sandbox that blocks git-spawning tests |

...
| Local lesson-gate probe | a synthetic ACTIVE capsule with one friction candidate surfaced the candidate in `krn-codex state resume`; flipping it to `COMPLETE` made `krn-codex state check` fail with `complete-with-friction` (exit 1); a second probe showed `state resume` printing a repository workflow-lessons page, and both probe artifacts were removed | keep the capsule field, the completion block, and resume-time lesson loading; the cross-run page stays the destination for dispositioned lessons | synthetic probes only; they do not prove lessons are promoted well after a real outcome, nor that the lessons page improves future behavior |
...
| Precompiled restart brief over the outcome capsule | lab-test; implemented as read-only `state compile` / `state resume` with focused tests in `npm run test:state`; a 2026-09-10 low-effort `gpt-5.6-luna` probe resumed a synthetic ACTIVE capsule correctly through `state check` and `state resume` with no edits | `$delivery-loop` restart path; a fresh low-effort session must resume from one compiled brief with no transcript reconstruction and no missing mechanical field, or the mechanism is deleted with its command | deterministic fields only; no background daemon, no always-on summarizer, no second memory store; the probe used one hand-authored synthetic capsule in one repository, so it does not prove recovery after a real long outcome, cross-family generality, or that the recorded state is semantically current |
| Procedural memory updates from repeated outcome friction | lab-test | `$delivery-loop` and maintainer on the next long outcome; one maintenance cycle must turn a repeated handler failure into a skill or reference edit, or the mechanism is deleted | no automatic skill rewriting, no reflection journal, no second lifecycle state |
...
| External-reference verification; intrinsic self-critique is not proof | adopt | every installed session; the proof budget states that a deterministic falsifier outranks a model review and same-model self-assessment is never proof, while `$delivery-loop` already requires a reviewer identity and model different from the implementer for an admissible disposition | no self-critique loop, no always-on judge, no second verification owner |
| MAST-derived review checklist for specification and verification failures | reject after the 2026-09-10 seeded-defect lab | deleted: low-effort `gpt-5.6-luna` found all three seeded defects with and without the checklist (control 3/3), so it added no detection at this difficulty; reopen only with a harder seed or a weaker reviewer that misses a defect without it | no always-loaded checklist and no multi-agent redesign; the taxonomy is descriptive, not KRN evidence; one seed and one model produced a ceiling effect |
...
| Proactive boundary compaction with a steering hint; addressable pointers over paraphrased summaries | adopt (already present) | `$delivery-loop` rewrites the capsule at every boundary, carries `Next bounded owner and action`, and the curation rule preserves provenance and pointers; no autocompact runs in KRN | no window-limit autocompact, no transcript archive, no second memory store |
| Subagent context isolation for noisy bounded work | defer | maintainer; reopen when a measured long outcome loses context or repeats exploration because of noisy tool output, then bound the delegation and its return shape | no subagent fleet or orchestration layer now |
| Boundary-local evaluation of each capsule rewrite (paired continuations from the same state) | defer | `$delivery-loop`; requires a real long outcome; reopen together with the procedural-friction lab once that outcome exists | no instrumentation and no always-on evaluator |
| Fresh-context evaluator with no write authority | adopt (refined in `$delivery-loop` step 4) | a review that shares the producing context or holds write authority for the outcome is not an admissible disposition; the already-required cross-model identity now names context too | no evaluator fleet and no extra review owner; same-context self-review may inform but never satisfies the gate |
| File-backed cross-run workflow-lesson memory (`Memory()` port) | adopt | `$delivery-loop` reads `docs/research/workflow-lessons.md` at bind and `krn-codex state resume` prints its lessons; every row carries evidence and an enforcing gate, and a capsule lesson candidate blocks `COMPLETE` until dispositioned | no API dependency, no shadow record, and no second store; `npm run validate` enforces the 24-row budget; a lesson without a gate is deleted |

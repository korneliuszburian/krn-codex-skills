...
| Conditional harness-adoption nudge and print-only onboarding output | defer | installed sessions in repositories without the managed contract; reopen only when a measured session neither discovers nor asks for the setup owner after bounded repair, or a nudge fires in an adopted repository | no auto-adoption, no global instruction writing, no session-start memory dump, and no expansion of the always-loaded contract budget |

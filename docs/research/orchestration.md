...
| Precompiled restart brief over the outcome capsule | lab-test; the vehicle is a planned `state compile` / `state resume` command pair | `$delivery-loop` restart path; a fresh low-effort session must resume from one compiled brief with no transcript reconstruction and no missing mechanical field, or the mechanism is deleted with its command | deterministic fields only; no background daemon, no always-on summarizer, no second memory store |

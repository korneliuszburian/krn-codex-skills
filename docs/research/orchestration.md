...
| Measurement track (`e2e-compare.mjs`, `lane-runner.mjs`, `opencode-agent.mjs`, `test/harness/tasks/`) | maintainer | LT-102 baseline at `aaad1db` and the `cube-block` skills ablation, plus `test/harness/*.test.mjs` | 2027-03-31 | the external scale (SWE-bench Verified slice + Terminal-Bench, LongMemEval-S + MemoryAgentBench CR) runs and each component is re-justified or retired, or the measurement is retired |

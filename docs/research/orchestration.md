...
| Local architecture review | a read-only sub-agent turned 14 findings into 7 deepening candidates; candidate 1 shipped at 88ab680 (`capsule-abi.mjs`, mutation 30/31 to 31/31), candidate 2 at a86fc28 (`lessons.mjs` `parseLessons`/`LESSON_BUDGET`, mutation 3/4 to 4/4), and candidate 3 at dd98bad (required `runtime_paths` with the stale fallback removed and one shared `classifyTarget` for preflight and doctor, mutation 2/3 to 3/3); independent luna reviews returned no finding with file:line citations and executed `npm run validate` | keep the report-driven, candidate-by-candidate pathway in the architecture-review capsule; candidates 4-7 remain | one reviewer model and a sandbox that blocks git-spawning state tests |

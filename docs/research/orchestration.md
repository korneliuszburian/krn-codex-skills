...
`$source-to-decision`. Owner: maintainer. Verified: 2026-09-13. Rework this page
...
| One shared capsule-discovery owner for `state check` and `spine-runs` | extract next (round-51 architecture finding: `state-check.mjs` `inspectSpineState` and `spine-runs.mjs` `capsuleIdsDetailed` validate the same store, entry containment, and `state.md` rules with separate code; the characterization is now pinned by `test/state/state-check.test.mjs` case "state check and spine discovery keep their distinct capsule diagnostics") | extract one resolver in `spine-runs` with per-consumer error adapters, keeping the new characterization case green; reopen the bound only if the case cannot be kept green | the extraction must not change either consumer's diagnostics or dedup (the two differ today: `inspectSpineState` dedups a resolved directory before containment and reports `${name} cannot be resolved`, while `capsuleIdsDetailed` has no id on errors and reports the relative `state.md` path); an unproven substitution that alters a warning or duplicate-error count is worse than the duplication |

...
| Local lesson-gate probe | a synthetic ACTIVE capsule with one friction candidate surfaced the candidate in `krn-codex state resume`; flipping it to `COMPLETE` made `krn-codex state check` fail with `complete-with-friction` (exit 1); the probe capsule was removed and the repository returned clean | keep the capsule field and the completion block; the cross-run page stays the destination for dispositioned lessons | one synthetic capsule; it does not prove lessons are promoted well after a real outcome, nor that the lessons page improves future behavior |
...
| File-backed cross-run workflow-lesson memory (`Memory()` port) | adopt | `$delivery-loop` reads `docs/research/workflow-lessons.md` at bind; every row carries evidence and an enforcing gate, and a capsule lesson candidate blocks `COMPLETE` until dispositioned | no API dependency, no shadow record, and no second store; `npm run validate` enforces the 24-row budget; a lesson without a gate is deleted |

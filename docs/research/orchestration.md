...
| Executor policy: Codex-only lanes | adopt (user decision 2026-09-16: Codex is KRN's only executor; no other transport or model family is authorized for KRN lanes, labs, or worker sessions) | installed sessions and lab runners; a lane or reviewer that would need another transport stops and reports instead of substituting one | no cross-transport or cross-family lanes, no second-executor support, and no model-pool machinery; this supersedes the LT-5 opencode-transport family for future runs, while the historical results stay as recorded |
| Executor portability: opencode | defer (assessment 2026-09-16, from the review session: the deterministic half is executor-agnostic — `krn-codex` gates, `~/.agents/skills` and per-repository `.agents/skills` discovery, the managed repository block through the project `AGENTS.md`, the change-contract enforced in CI, and the publication flow all run under opencode; the Codex-bound parts are the always-loaded global contract, the SessionStart/PreCompact hooks, the PreToolUse guard, the onboarding signal, and the hook-injected worker continuation) | maintainer deciding an executor extension; reopen only when the user authorizes it, then port the global contract into opencode's instruction path and the hooks into its plugin layer and re-run one lane under opencode with the same gates | no second-executor support before that decision and no proxy claims from the executor-agnostic subset |

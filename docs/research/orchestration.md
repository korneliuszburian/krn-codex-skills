...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; catalog-usage was split at 190dd42 into catalog-usage-json (264) and at f76277d into catalog-usage-shell (131), leaving a 562-line scanner; direct tests cover planning, JSON extraction, path normalization, and shell reads with mutation falsifiers shown red then green; luna reviews raised only formatting and test-coverage findings, repaired and re-reviewed | continue with catalog-inventory, then the validate rule remainder | catalog-usage and catalog-inventory remain large untested modules, catalog-document can still split parser from operations, and review sandboxes block git-spawning tests |

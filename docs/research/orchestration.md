...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; catalog-usage was split into catalog-usage-json (264), catalog-usage-shell (131), catalog-usage-normalize (201), catalog-usage-paths (80), and catalog-usage-lines (92), leaving a 218-line scanner with window/malformed/undated/oversize/symlink branch coverage and an end-to-end canonical-skill-read test that proves per-path confidence classification and merged escalation; catalog-inventory was split into catalog-profiles (237), catalog-inventory-roots (79), catalog-inventory-frontmatter (23), and catalog-inventory-quarantine (67), leaving a 536-line adapter covered by frontmatter, plugin-cache, quarantine/symlink, manifest-fallback, and a symlink-target manifest test that proves O_NOFOLLOW, with a lost-quarantine-record defect fixed at fb21b63; validate was split into content-rules (235), upstream-sources (71) plus path-rules (10), manifest-rules (94), delivery-loop-rules (42), and skill-rules, leaving a 365-line remainder; catalog-desired and catalog-profile (the latter at 8462bf8) gained direct tests; a dead-export audit at 7e5c7dc narrowed 19 internal helpers behind a module-surface guard; the git-adapter dedupe introduced and then fixed a missing gitAvailable import; a mechanical quality audit at 428d8fa is guarded by test/quality-audit.test.mjs; direct tests cover planning, TOML parsing, document operations, JSON extraction, shell reads, normalization, path rules, the rollout line reader, profile resolution, desired-state rules, profile validation, root resolution, frontmatter, quarantine, path safety, content rules, upstream rules, manifest rules, skill rules, hook, retirement and bin rules, delivery-loop artifact rules, branch-level scan tests, canonical skill reads, and plugin-cache and quarantine integration, with mutation falsifiers shown red then green; a read-only review missed an undefined identifier introduced by one extraction, so an end-to-end scan test now guards runtime reachability, broader executing coverage exposed a silently dropped quarantine record, and the quality audit now catches the undefined-call class mechanically | continue with catalog-config integration coverage | catalog-config still has no direct tests and luna sandboxes report false test:skills, test:composition, test:install, and test:state failures that pass locally |

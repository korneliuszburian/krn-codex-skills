...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; catalog-usage was split at 190dd42 into a 688-line scanner and catalog-usage-json (264); first direct catalog tests cover planning and JSON extraction with mutation falsifiers shown red then green; luna reviews raised only formatting and test-coverage findings, repaired and re-reviewed | continue with the catalog-usage shell block, catalog-inventory, then the validate rule remainder | catalog-usage and catalog-inventory remain large untested modules, catalog-document can still split parser from operations, and review sandboxes block git-spawning tests |

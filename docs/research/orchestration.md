...
| Conditional harness-adoption signal and read-only adoption report | lab-test (KRN-native, implemented 2026-09-16: the SessionStart hook emits one advisory line only when a work tree carries agent instructions without the managed block and no capsule continues, and `krn-codex repo inspect` reports `managedContract`; `test:hooks` and `test:setup` cover firing, silence, and the report) | installed sessions in unadopted work trees; the discovery-effect claim stays unmeasured until a bounded pilot shows whether a signaled session reads the report or asks its owner, and a signal that fires in an adopted repository or without agent instructions fails it | no per-session prime, no auto-adoption, no global instruction writing, no memory dump, and no expansion of the always-loaded contract budget |

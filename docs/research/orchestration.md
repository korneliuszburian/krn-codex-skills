...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; catalog-usage was split at 190dd42 into catalog-usage-json (264), at f76277d into catalog-usage-shell (131), at 83101ae into catalog-usage-normalize (201), and at f73fdb8 into catalog-usage-paths (80), leaving a 303-line scanner; catalog-inventory at 4d11601 was split into catalog-profiles (237) and at 72e26b2 into catalog-inventory-roots (79), leaving a 618-line adapter; validate was split at a67debb into content-rules (235), at 9384742 into upstream-sources (71) plus path-rules (10), and at 06ef341 into manifest-rules (94), leaving a 497-line remainder; direct tests cover planning, JSON extraction, shell reads, normalization, path rules, profile validation, root resolution, content rules, upstream rules, and manifest rules with mutation falsifiers shown red then green; a read-only review missed an undefined identifier introduced by one extraction, so an end-to-end scan test now guards runtime reachability | continue with the catalog-inventory frontmatter readers | catalog-usage and catalog-inventory remain large partly-untested modules, catalog-document can still split parser from operations, and luna sandboxes report false test:skills and test/composition failures that pass locally |

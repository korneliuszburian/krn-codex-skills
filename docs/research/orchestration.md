...
| Local architecture review | a read-only sub-agent turned 14 findings into 7 deepening candidates (capsule ABI, lessons schema, install trust, validator rules, diagnostics, export index, catalog identity); candidate 1 shipped at 88ab680 as `scripts/lib/capsule-abi.mjs` (mutation 30/31 red then 31/31) and candidate 2 shipped at a86fc28 as `scripts/lib/lessons.mjs` `parseLessons`/`LESSON_BUDGET` (mutation 3/4 red then 4/4); independent luna reviews returned no finding with file:line citations, one ran `npm run validate` and the lessons tests | keep the report-driven, candidate-by-candidate pathway in the architecture-review capsule; candidates 3-7 remain | one reviewer model and a sandbox that blocks git-spawning state tests |

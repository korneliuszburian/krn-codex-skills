...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter; catalog-usage was split at 190dd42 into catalog-usage-json (264), at f76277d into catalog-usage-shell (131), at 83101ae into catalog-usage-normalize (201), at f73fdb8 into catalog-usage-paths (80), and at ecee034 into catalog-usage-lines (92), leaving a 218-line scanner with branch coverage including symlinked and forbidden rollout entries; catalog-inventory at 4d11601 was split into catalog-profiles (237), at 72e26b2 into catalog-inventory-roots (79), at b03b46f into catalog-inventory-frontmatter (23), and at a05ce29 into catalog-inventory-quarantine (67), leaving a 536-line adapter covered by frontmatter, plugin-cache, quarantine/symlink, and manifest-fallback integration tests, with a lost-quarantine-record defect fixed at fb21b63; validate was split at a67debb into content-rules (235), at 9384742 into upstream-sources (71) plus path-rules (10), at 06ef341 into manifest-rules (94), at c74d767 into delivery-loop-rules (42), at b0164be adding hook and retirement rules, at 8612682 adding bin rules, at 84084c1 adding hook-file and legacy-path rules, and at 1bcc293 adding skill metadata, content, pointer, and line-limit rules (skill-rules), leaving a 384-line remainder; a dead-export audit at 7e5c7dc narrowed 19 internal helpers behind a module-surface guard; the git-adapter dedupe introduced and then fixed a missing gitAvailable import; a mechanical quality audit at 428d8fa (dead exports, cross-module calls without imports, unreferenced functions, duplicate bodies, smells, unused lib files) is now guarded by test/quality-audit.test.mjs; overall direct tests cover planning, TOML parsing, document operations, JSON extraction, shell reads, normalization, path rules, the rollout line reader, profile validation, root resolution, frontmatter, quarantine, path safety, content rules, upstream rules, manifest rules, skill rules, hook, retirement and bin rules, delivery-loop artifact rules, branch-level scan tests, and plugin-cache and quarantine integration, with mutation falsifiers shown red then green; a read-only review missed an undefined identifier introduced by one extraction, so an end-to-end scan test now guards runtime reachability, broader executing coverage exposed a silently dropped quarantine record, and the quality audit now catches the undefined-call class mechanically | continue with the validate directory and reference rules | catalog-inventory still lacks unreadable-file coverage and luna sandboxes report false test:skills, test:composition, test:install, and test:state failures that pass locally |

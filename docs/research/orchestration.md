...
| Local monolith decomposition | catalog-config at 1352 lines was split into catalog-errors (28), catalog-document (580), catalog-desired (265), catalog-plan (264), and a 238-line adapter, the latter now covered end to end at 1fea762 with a strengthened stale-plan assertion at 54f38bf; catalog-usage was split into catalog-usage-json (264), catalog-usage-shell (131), catalog-usage-normalize (201), catalog-usage-paths (80), and catalog-usage-lines (92), leaving a 218-line scanner with full branch coverage and a per-path confidence proof; catalog-inventory was split into catalog-profiles (237), catalog-inventory-roots (79), catalog-inventory-frontmatter (23), and catalog-inventory-quarantine (67), leaving a 536-line adapter covered by frontmatter, plugin-cache, quarantine/symlink, manifest-fallback, and O_NOFOLLOW tests, with a lost-quarantine-record defect fixed at fb21b63; validate was split into content-rules (235), upstream-sources (71) plus path-rules (10), manifest-rules (94), delivery-loop-rules (42), and skill-rules, leaving a 365-line orchestrator body; a dead-export audit at 7e5c7dc narrowed 19 internal helpers behind a module-surface guard; the git-adapter dedupe introduced and then fixed a missing gitAvailable import; a mechanical quality audit at 428d8fa is guarded by test/quality-audit.test.mjs; every runtime lib module now has a direct or integration test, with mutation falsifiers shown red then green; a read-only review missed an undefined identifier introduced by one extraction, so an end-to-end scan test now guards runtime reachability, broader executing coverage exposed a silently dropped quarantine record, and the quality audit now catches the undefined-call class mechanically | run the no-candidate completion survey | catalog-config identity-race windows are nondeterministic without injection and luna sandboxes report false test:skills, test:composition, test:install, and test:state failures that pass locally |

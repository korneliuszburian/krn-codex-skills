...
| `unlazy` harness | its current source puts gates, explicit evidence, re-verification, a task tree, and cooperative leases before or around work; its own boundary notes distinguish coordination from isolation | retired 2026-09-15 (manifest `retired_skills`) pending a second measured real consumer; the design record stays in `docs/research/unlazy-codex-port.md` | source design and historical self-reports never proved lower cost, better outcomes, or hostile-process isolation |

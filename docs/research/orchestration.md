...
| Held-out or composition acceptance check | lab-test | `$delivery-loop` on the next non-trivial outcome; the visible feature test can saturate while composed usage fails (SpecBench), so one check the producer did not author must be added or the outcome stays `NEEDS_REVIEW` | no second test suite framework; the lab deletes the rule if it adds ceremony without catching a composition gap |
...
| Per-slice passing criterion before advancing | lab-test | `$slice-work` and `$delivery-loop` on the next multi-slice outcome; each slice must name the check that decides it before work starts, or the rule is deleted | no DAG engine, no scheduler, and no second spec store |

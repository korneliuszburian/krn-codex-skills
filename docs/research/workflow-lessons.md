...
| Duplicated low-level adapters drift; centralize once and alias at call sites. | Four copies of a git wrapper with two different contracts lived across `state-check`, `state-brief`, `skills-export`, and `install-release`; one `git-cli` module with `runGit`/`gitText`/`gitAvailable` now backs all four. | `test/support/git-cli.test.mjs`. | | `test/support/git-cli.test.mjs::runGit returns trimmed output inside a repo and fails safely outside@7c826bc` | |

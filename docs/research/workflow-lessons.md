...
| A budgeted surface accepts a new rule only after displacing or condensing existing text. | `delivery-loop/SKILL.md` reached 181 lines and `validate` failed until the fresh-context and lesson-candidate rules were condensed back to 179. | The line ceilings in `validate.mjs` (90 source `AGENTS.md`, 60-element installed contract, 180 per skill). |
...
| A prose rule that no check enforces is ignored at low effort. | The explicit-only rule was ignored by a low-effort model until it was narrowed and behaviorally checked; per-slice and held-out rules now have a test/inspection gate. | `$delivery-loop` step 4 review rejects a prose rule without its check. |

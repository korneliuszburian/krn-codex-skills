...
| Render CLI diagnostics as text, never raw objects. | `krn-codex state check` printed `warning: [object Object]` for the first structured warning. | The readable-warning test in `test/state/state-check.test.mjs`. | | | | retired@777837c; enforced-by:test/state/state-check.test.mjs |
...
| Blocking errors must be visible in human command output, not only in the exit code. | A composition probe showed `state resume` exiting 1 on `lessons-over-budget` while printing no reason. | The blocking-errors brief test in `test/state/state-brief.test.mjs`. | | | | retired@777837c; enforced-by:test/state/state-brief.test.mjs |
| A prose rule that no check enforces is ignored at low effort. | The explicit-only rule was ignored by a low-effort model until it was narrowed and behaviorally checked; per-slice and held-out rules now have a test/inspection gate; the ticket ABI's `Scope` field was equally inert, so the sh-1 lane added `skills/manifest.json` while Scope named four other paths, until `krn-codex ticket check --id --base` made it structural (`scope-undeclared`). | `manual:review`; `scripts/lib/lessons/lessons.mjs`; `test/ticket/ticket-scope.test.mjs`. | 2026-09-16@082b237 | `test/ticket/ticket-scope.test.mjs::checkTickets reports changed files outside the ticket Scope@e6c2c41` | |
| Hand-written capsule fixed points must use full commit tokens. | A hand-filled capsule with short hashes failed `state check` with `invalid-fixed-point` until the full tokens were used. | `test/state/state-check.test.mjs`. | | | | retired@777837c; enforced-by:test/state/state-check.test.mjs |

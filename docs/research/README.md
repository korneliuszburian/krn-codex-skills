...
|---|---|---|---|
| Agent orchestration and compact context | [orchestration.md](orchestration.md) | accepted compiled-context spine; bounded register lab did not earn a new store; retrieval stays a composed ladder without a vector store; deterministic spine check covers structural capsule divergence; upstream harness mechanisms are selectively adopted or lab-tested; OpenCode advisory transport is advisory-only with a verified runner contract and no scope isolation; history hygiene is owned by the always-loaded contract, and measured 2026-09-11 staleness/reachability failures now earn fail-closed capsule, gate-resolution, knowledge-map, and orphaned-run checks instead of warnings | a recurring routing, restart, review, artifact, writer-admission, retrieval, advisory-transport, or history-hygiene failure survives bounded repair |
...
| Lab tests | [lab-tests.md](lab-tests.md) | mandatory blinded pilots and their registry; a behavioral claim stays `lab-test` until a registered pilot reports a result with non-proofs | a promotion claim lacks a pilot, a pilot result is contradicted, or a new behavioral mechanism is proposed without a lane design |

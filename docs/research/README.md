...
| Three-arm skills lab | [skills-3arm-lab.md](skills-3arm-lab.md) | no measured advantage of a hand-forked skill over upstream or no-skill; only aggregate findings are retained locally | a new controlled comparison changes the fork decision or a replayable evidence record is deliberately created |
| Beads task system | [beads-task-system.md](beads-task-system.md) | optional task-graph adapter remains lab-test; it does not own lifecycle, authority, or publication | an isolated multi-session pilot measures less duplicate work without adding ceremony |

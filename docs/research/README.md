...
|---|---|---|---|
| Agent orchestration and compact context | [orchestration.md](orchestration.md) | accepted compiled-context spine; bounded register lab did not earn a new store; retrieval stays a composed ladder without a vector store; deterministic spine check covers structural capsule divergence; upstream harness mechanisms are selectively adopted or lab-tested; OpenCode advisory transport is advisory-only with a verified runner contract and no scope isolation | a recurring routing, restart, review, artifact, writer-admission, retrieval, or advisory-transport failure survives bounded repair |

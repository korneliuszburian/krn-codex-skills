...
implementation and LT-8 release decision. Delete this page after FH-15 completes
...
| Generated CUBE snapshot and frontend procedures | `frontend-library` and sibling skills in KRN, through the importer only | project agents and evaluator tasks |
...
- **Dependencies:** blocked by FH-01's manifest contract. Blocks FH-06, FH-11,
  and FH-12.
...
- **Entry → exit:** v1 tasks continue to load → v2 declares track, public
  ...
  paths, evidence, and multi-axis result without changing v1 behavior.
- **Fastest falsifier:** malformed track, evaluator inside public workspace,
  hidden requirement, missing environment identity, or weighted aggregate is
  refused; existing v1 checks remain green.
- **Decision carried:** hidden answer but no hidden requirement; two tracks; no
  magic score.
- **Dependencies:** none. Blocks FH-05, FH-07, FH-08, and FH-09.
- **Rollback:** versioned loader dispatch preserves v1.

### FH-05 — Expand: isolated runner and environment digest

- **Repository:** KRN.
- **Entry → exit:** the runner copies one visible workspace → v2 copies only
  public input, proves oracle/sentinel isolation, records network behavior,
  freezes output before external evaluation, and attaches a pinned environment
  digest.
- **Fastest falsifier:** candidate reads the oracle sentinel, evaluator resolves
  under the public workspace, network violates policy, or browser/font/locale
  drift retains the old digest.
...
- **Dependencies:** blocked by FH-04. Blocks FH-07, FH-08, FH-09, FH-11, FH-12.
...
- **Entry → exit:** source and skill copies disagree → every file has an
  ...
  and the skill snapshot is regenerated with browser/build behavior preserved.
- **Fastest falsifier:** source export plus KRN import checks catch deliberate
  drift in `flow`, `grid`, `wrapper`, `text`, tokens, or a stable block.
...
- **Dependencies:** blocked by FH-01, FH-02, and FH-03. Blocks FH-07 and FH-10.
...
### FH-07 — Migrate: canonical architecture evaluator axis
...
- **Dependencies:** blocked by FH-05 and FH-06. Blocks FH-11 and FH-12.
- **Rollback:** the new axis is additive; current project audit remains the gate
  until FH-15.

### FH-08 — Migrate: responsive, interaction, and accessibility state paths
...
- **Dependencies:** blocked by FH-04 and FH-05. Blocks FH-11 and FH-12.
- **Rollback:** existing signed browser evidence remains valid; v2 uses a
  versioned schema.

### FH-09 — Migrate: geometry and visual evidence axis
...
- **Dependencies:** blocked by FH-04 and FH-05. Blocks FH-11 and FH-13.
- **Rollback:** design-transfer reports visual evidence without granting it sole
  pass authority.

### FH-10 — Migrate: deep Text profiles and smart ACF normalization
...
- **Fastest falsifier:** duplicate fields/renderers, raw CSS choice, profile-
  invalid value, known legacy token, and a newly admitted real design role take
...
- **Dependencies:** blocked by FH-02, FH-05, FH-07, FH-08, and FH-09. Blocks
  FH-13 and FH-14.
- **Rollback:** tasks coexist with `cube-block` until FH-15 proves no reader.

### FH-12 — Migrate: WordPress/ACF adapter task pack
...
- **Decision carried:** frontend and CMS adapter are separate axes; Flexible
  ...
  the baseline.
- **Dependencies:** blocked by FH-02, FH-05, FH-07, FH-08, and FH-10. Blocks
  FH-13 and FH-14.
- **Rollback:** only disposable WordPress state is touched; no developer or
  production database is mutated.

### FH-13 — Migrate: bounded repair and paired raw reporting
...
- **Entry → exit:** the harness aggregates pass/tokens/wall → v2 retains raw
  ...
  input/output/reasoning tokens, latency, retries, and artifact identities.
- **Fastest falsifier:** unconditional repair, mixed cohort aggregate, repair
  ...
  worse worst viewport is rejected.
- **Decision carried:** external verification beats self-critique; efficiency is
  evidence, never a source-code budget.
- **Dependencies:** blocked by FH-09, FH-11, and FH-12. Blocks FH-14.
- **Rollback:** generic v1 summaries stay available during migration.

### FH-14 — Migrate: LT-8 feasibility pilot and powered-run decision

- **Repositories:** KRN protocol/result plus lab runner and private raw outputs.
...
- **Fastest falsifier:** pre-run seal, sentinel/network, task mutation, project-
  creation, and pair-completeness checks must pass before any uplift statistic is
  read.
...
- **Dependencies:** blocked by FH-14 and any migrate unit that still names an old
  consumer.
...
FH-01 -> FH-02 -------------------------------> FH-11/FH-12
   |       |
   +-----> FH-03 -> FH-06 -> FH-07 -> FH-11 -> FH-13 -> FH-14 -> FH-15
                         |       |       ^         ^
                         |       |       |         |
                         +----> FH-10 -> FH-12 ----+
...
they have different repositories and seams. FH-08 and FH-09 may proceed
independently after runner isolation. FH-10 has a distinct boilerplate writer.
...
- **Migrate exit:** FH-06 through FH-14 are green; every task has known-good and
  targeted-mutant evidence; LT-8 reports results and non-proofs.
- **Contract exit:** FH-15 proves no old reader, deletes obsolete authority, and
...
rule in the opening paragraph after FH-15 and the release decision are complete.

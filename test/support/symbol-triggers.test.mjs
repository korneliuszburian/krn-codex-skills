...
test("extractSymbols reads every declarator and ignores default-expression identifiers", () => {
  assert.deepEqual(extractSymbols("export const a = 1, b = 2;\n").map((symbol) => symbol.name), ["a", "b"]);
  assert.deepEqual(
    extractSymbols('export const { a = foo, b = "phoney" } = src;\n').map((symbol) => symbol.name),
    ["a", "b"],
  );
...
test("touchedSymbols maps changed lines to the symbol they fall inside", () => {
  const git = strictGit("--- a/scripts/lib/kernel/git.mjs\n+++ b/scripts/lib/kernel/git.mjs\n@@ -2,0 +3,1 @@\n", {
    "abc:scripts/lib/kernel/git.mjs": "export function runGit(r) {\n  return 1;\n}\nexport const OTHER = 1;\n",
  });
